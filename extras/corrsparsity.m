...
if options.sos.csp
    ...
    % -------------------------------------------------------
    % Matrix C is the correlative sparsity pattern
    % The following is to identify the maximal cliques
    % D is a cell struture with all maximal cliques
    %     s = symamd(C);
    %     R = chol(C(s,s)+100*speye(n));
    %     % -------------------------------------------------------
    %     for i = 1:size(R,1)
    %         [row,col] = find(R(i,:));
    %         % ------------------------------
    ...
    clique = cliquesFromSpMatD(sparse(C));
    D = clique.Set;  % cliques
